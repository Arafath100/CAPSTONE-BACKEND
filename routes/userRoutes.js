...
router.post('/signup', express.json(), async function (request, response) {
  ...
  if (userPresentOrNot == null) {
    ...
    tokenExpirationTime.setHours(tokenExpirationTime.getHours() + 2); // Set expiration time to 2 hour from now

    const token = randomstring.generate();
    const verificationInitiate = await addToken({
      email: email,
      token: token,
      expirationTime: tokenExpirationTime, // Include expiration time in the token data
      DateTime: new Date(),
    });
